...
;(function($, win, doc) {
	...
	$.anythingSlider = function(el, options) {
		...
		base.init = function(){
			...
			base.currentPage = base.targetPage = o.startPanel = (parseInt(o.startPanel,10) || 1) + 1; // make sure this isn't a string
			...
			base.currentPage = ((o.hashTags) ? base.gotoHash() : '') || o.startPanel || 2;
		...
		base.updateSlider = function(){
			...
			base.currentPage = base.currentPage || 2;
			...
			if (o.mode !== 'fade' && o.infiniteSlides && base.pages > 1) {
				base.$el.prepend( base.$items.filter(':last, :eq(' + (base.$items.length - 2) + ')').clone().addClass('cloned') );
				// Add support for multiple sliders shown at the same time
				if (o.showMultiple > 1) {
					base.$el.append( base.$items.filter(':lt(' + o.showMultiple + ')').clone().addClass('cloned multiple') );
				} else {
					base.$el.append( base.$items.filter(':first').clone().addClass('cloned') );
					base.$el.append( base.$items.filter(':eq(1)').clone().addClass('cloned') );
				}
			...
			if (base.currentPage > base.pages + 1) {
				base.currentPage = base.pages + 1;
			}
			base.setCurrentPage(base.currentPage, false);
		...
		base.buildNavigation = function() {
			if (o.buildNavigation && (base.pages > 1)) {
				...
				base.$items.filter(':not(.cloned)').each(function(j){
					$li = $('<li/>');
					i = j + 2;
					c = (i === 2 ? ' first' : '') + (i === base.pages + 1 ? ' last' : '');
				...
				base.$nav.children('li').bind(o.clickControls, function(e) {
					if (!$(this).children("a").hasClass("cur") && !base.flag && o.enableNavigation) {
		...
		base.getDim = function(page){
			...
			if (base.pages < 2 || isNaN(page)) { return [ w, h ]; } // prevent errors when base.panelSize is empty
		...
		base.gotoPage = function(page, autoplay, callback, time) {
			...
			if (o.changeBy !== 1){
				...
				if (page < 2) {
					page = o.stopAtEnd ? 2 : ( o.infiniteSlides ? base.pages + 1 + page : ( o.showMultiple > 2 - page ? 2 : adj ) );
				}
				if (page > base.pages + 1) {
					//
					page = o.stopAtEnd ? base.pages + 1 : ( o.showMultiple > 2 - page ? 2 : page -= adj );
				} else if (page >= adj) {
			...
			base.exactPage = page;
			if (page > base.pages + 2 - base.adj) { page = (!o.infiniteSlides && !o.stopAtEnd) ? 2 : base.pages + 1; }
			if (page < base.adj ) { page = (!o.infiniteSlides && !o.stopAtEnd) ? base.pages + 1 : 2; }
			if (!o.infiniteSlides) { base.exactPage = page; } // exact page used by the fx extension
			base.currentPage = ( page > base.pages + 1 ) ? base.pages + 1 : ( page < 2 ) ? 2 : base.currentPage;
			base.$currentPage = base.$items.eq(base.currentPage - base.adj);
			base.targetPage = (page === 1) ? base.pages + 1 : (page > base.pages + 1) ? 2 : page;
			...
			if (!autoplay || (o.stopAtEnd && page === base.pages + 1)) { base.startStop(false); }
		...
		base.setCurrentPage = function(page, move) {
			...
			if (base.pages < 1 || page < 2 || isNaN(page)) { return; }
			if (page > base.pages + 2 - base.adj) { page = base.pages + 1 - base.adj; }
			if (page < base.adj ) { page = 2; }

			// hide/show arrows based on infinite scroll mode
			if (o.buildArrows && !o.infiniteSlides && o.stopAtEnd){
				base.$forward[ page === base.pages + 1 - base.adjustMultiple ? 'addClass' : 'removeClass']('disabled');
				base.$back[ page === 2 ? 'addClass' : 'removeClass']('disabled');
				if (page === base.pages + 1 && base.playing) { base.startStop(); }
			}
